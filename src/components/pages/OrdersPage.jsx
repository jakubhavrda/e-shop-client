  ...
  const getOrder = async () => {
    try {
      const response = await fetch(
        `http://localhost:4000/order/${params.user_id}/${params.order_id}`
      );
    ...
    <Fragment>
      ...
      {order.map((x) => (
        <div>
          ...
          <h4>
            Total: <span className="text-danger">{x.total_price} kč</span>
          </h4>
          <p>paid: {x.paid ? "✔" : "❌"}</p>
          <p>complete: {x.complete ? "✔" : "❌"}</p>
          <a hidden={authorizeOrder()} href="/checkout">
            <button className="btn btn-lg btn-warning mb-5">
              Finish Order
            </button>
          </a>
